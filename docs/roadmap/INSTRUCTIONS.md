...
- Added local PostgreSQL runtime via `platform/docker/docker-compose.yml` with one DB per service.
...
1. `docker compose -f platform/docker/docker-compose.yml up -d`
