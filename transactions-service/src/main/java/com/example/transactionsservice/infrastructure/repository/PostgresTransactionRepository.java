...
public class PostgresTransactionRepository implements TransactionRepository {
  ...
  private static Transaction mapTransaction(io.r2dbc.spi.Row row) {
  ...
  // ---------------------------------------------------------------------------
  // Window function: running balance
  // ---------------------------------------------------------------------------
  ...
  // ---------------------------------------------------------------------------
  // Ranking: top accounts by volume
  // ---------------------------------------------------------------------------
  ...
  public Flux<Transaction> search(
    ...
    // Build WHERE clause dynamically; only add predicates for non-null params.
    ...
    return spec.map((row, meta) -> mapTransaction(row)).all();
  }
}
